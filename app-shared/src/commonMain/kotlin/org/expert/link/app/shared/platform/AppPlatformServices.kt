...
private fun unsupported(message: String): Result<Unit> = Result.failure(UnsupportedOperationException(message))
...
interface AppPlatformServices {
    ...
    suspend fun copyText(label: String, text: String): Result<Unit> = unsupported("$label нельзя скопировать на этой платформе")

    /** Пытается передать текст через системный share flow. */
    suspend fun shareText(label: String, text: String): Result<Unit> = unsupported("$label нельзя отправить с этой платформы")

    /** Открывает системный выбор файла. */
    suspend fun pickFile(): Result<String?> = Result.success(null)

    /** Строит QR-код для строки invite, если платформа это поддерживает. */
    fun buildQrCode(text: String): QrCodeMatrix? = null
}
